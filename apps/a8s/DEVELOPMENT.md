...
- **Tell outbox resolution.** `TELL_OUTBOX_DIR` when set (a8s injects it on wake). Else a unique configured outbox matched from CWD when `~/.a8s` is readable (see `docs/filedrop.md`). `install-client` tell-only has no registry — env required. No blind CWD tree-walk for a random `.outbox`.
