...
| `history_max_bytes` / `history_body_max` / `prompt_body_max` (rig) | 8192 / 2000 / 4000 | Context sizing on the rig: rolling-history budget, per-entry history clip, and per-message prompt clip. Raise for a high-context rig (agy), lower for a small local model | A weak rig drowning in context, or a strong one starved of it |
